...
import com.diffbot.wikistatsextractor.extractors.ExtractSFAndRedirections;
...
public class Util {
	...
	public static String getNResults(String optional_language) {
		String n_results="";
		String wikiDataEndpoint = "http://wikidata.demo.openlinksw.com/sparql";

		String query = "PREFIX schema: <http://schema.org/>\n" + 
				"SELECT (COUNT(?uri) AS ?results)\n" + 
				"WHERE{\n" + 
				" ?article schema:isPartOf <https://" + optional_language +".wikipedia.org/> .\n" + 
				" ?article schema:name ?uri ."+
				"}";

		QueryExecution q = QueryExecutionFactory.sparqlService(wikiDataEndpoint, query); 
		ResultSet results = q.execSelect();
		while (results.hasNext()) {
			QuerySolution soln = results.nextSolution();
			n_results = soln.get("?results").toString();
		}
		n_results= n_results.replace("^^http://www.w3.org/2001/XMLSchema#integer", "");
		return n_results;
	}
	
	public static Map<String, String> loadWikidataResources(String optional_language){

		    Map<String, String> map = new HashMap<String,String>();
		
			System.out.println("Wikidata not loaded");
			String wikiDataEndpoint = "http://wikidata.demo.openlinksw.com/sparql";

			int offset = 0;
			int n_results = Integer.parseInt(getNResults(optional_language));
			int iterations = (n_results/100000) +1;
			int i=0;

			String aux = "PREFIX schema: <http://schema.org/>\n" + 
					"SELECT ?uri ?wikidata_resource\n" + 
					"WHERE{\n" + 
					" ?article schema:about  ?wikidata_resource.\n" + 
					" ?article schema:isPartOf <https://" + optional_language +".wikipedia.org/> .\n" + 
					" ?article schema:name ?uri ."+
					"}\n"+
					"LIMIT 100000\n";
			...
			System.out.println("Loading wikidata...");
			while(i<iterations) {
				QueryExecution q = QueryExecutionFactory.sparqlService(wikiDataEndpoint, query);
				ResultSet results = q.execSelect();
				while (results.hasNext()) {
					QuerySolution soln = results.nextSolution();
					String resource = soln.get("?wikidata_resource").toString();
					String uri = soln.get("?uri").toString().replace("@"+ optional_language, "");
				    map.put(uri, resource);
				}
				i+=1;
				offset += 100000;
				query= aux + "OFFSET " + offset;
				System.out.println((iterations -i) + " iterations left");
				q.close();
			}
			System.out.println(map.size() + " resources loaded!");
