...
object DBpediaResourceSource {
	...
	def fromPigInputStreams(
			...
			dbpedia_instanceTypes: (String, InputStream),
			wikidata_instanceTypes: (String, InputStream),
			namespace: String
			): java.util.Map[DBpediaResource, Int] = {
					...
					//Read types:
					if (dbpedia_instanceTypes != null && dbpedia_instanceTypes._1.equals("tsv") && wikidata_instanceTypes != null && wikidata_instanceTypes._1.equals("tsv")) {
						SpotlightLog.info(this.getClass, "Reading types (tsv format)...")
						val uriNotFound = HashSet[String]()
						Source.fromInputStream(dbpedia_instanceTypes._2).getLines() foreach {
							line: String => {
						...
						Source.fromInputStream(wikidata_instanceTypes._2).getLines() foreach {
							line: String => {
								val Array(uri: String, typeURI: String) = line.trim().split('\t')
					...
					else if (dbpedia_instanceTypes != null && dbpedia_instanceTypes._1.equals("nt") && wikidata_instanceTypes != null && wikidata_instanceTypes._1.equals("nt")) {
						SpotlightLog.info(this.getClass, "Reading types (nt format)...")

						val uriNotFound = HashSet[String]()

						val dbpedia_redParser = new NxParser(dbpedia_instanceTypes._2)
						...
						val wikidata_redParser = new NxParser(wikidata_instanceTypes._2)
	...
	def fromPigFiles(
			wikipediaToDBpediaClosure: WikipediaToDBpediaClosure,
			counts: File,
			dbpedia_instanceTypes: File,
			wikidata_instanceTypes: File,
			namespace: String
			): java.util.Map[DBpediaResource, Int] = fromPigInputStreams(
					wikipediaToDBpediaClosure,
					new FileInputStream(counts),
					if(dbpedia_instanceTypes == null)
						null
						else
							(
									if(dbpedia_instanceTypes.getName.endsWith("nt")) "nt" else "tsv",
									new FileInputStream(dbpedia_instanceTypes)
									),
							if(wikidata_instanceTypes == null)
								null
								else
									(
											if(wikidata_instanceTypes.getName.endsWith("nt")) "nt" else "tsv",
											new FileInputStream(wikidata_instanceTypes)
											),
									namespace
					)
