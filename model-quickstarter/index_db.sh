...
if [ -f "$WP_DOWNLOAD_FILE" ]; then
  read -p "Dump exists but maybe it's out of date. Do you want to delete it and download it again? [y/n] " delete
	if [[ $delete == [yY] ]]; then 
		echo "Removing wikipedia dump"
		rm -f $WP_DOWNLOAD_FILE
		echo "Downloading wikipedia dump again."
		if [ "$eval" == "false" ]; then
...
cd $WDIR
echo "Loading Wikidata dump..."

WD_DOWNLOAD_FILE=$WDIR/latest-truthy.nt.bz2
echo Checking for wikidata dump at $WD_DOWNLOAD_FILE

if [ -f "$WD_DOWNLOAD_FILE" ]; then
  read -p "Dump exists but maybe it's out of date. Do you want to delete it and download it again? [y/n] " delete
	if [[ $delete == [yY] ]]; then 
		echo "Removing wikidata dump"
		rm -f $WD_DOWNLOAD_FILE
		echo Downloading wikidata dump again.
        	curl -# "https://dumps.wikimedia.org/wikidatawiki/entities/latest-truthy.nt.bz2" -o latest-truthy.nt.bz2
	fi
else
...
echo "Getting Wikidata artifacts"

echo "Obtaining redirects"
bzgrep "<http://www.w3.org/2002/07/owl#sameAs> <http://www.wikidata.org/entity/" latest-truthy.nt.bz2 > wikidata_redirects.nt
echo "Done"

echo "Obtaining disambiguations"
bzgrep "<http://www.wikidata.org/prop/direct/P1889> <http://www.wikidata.org/entity/" latest-truthy.nt.bz2 > wikidata_disambiguations.nt
echo "Done"

echo "Obtaining instance types"
bzgrep "<http://www.wikidata.org/prop/direct/P31> <http://www.wikidata.org/entity/" latest-truthy.nt.bz2 > wikidata_instance_types.nt
echo "Done"

echo "Wikidata artifacts available in ${WDIR}"
...
echo "Finished wikistats extraction."
